...
function renderList(id, rows, emptyKey) {
  ...
  if (!rows || !rows.length) {
    const art =
      emptyKey === "emptyBoard" || !emptyKey
        ? `<img class="empty-illust" src="./assets/empty-board.jpg?v=1" alt="" />`
        : "";
    el.innerHTML = `<li class="empty">${art}<span>${t(emptyKey || "emptyBoard")}</span></li>`;
    return;
  }
